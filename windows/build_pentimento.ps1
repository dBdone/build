...
$tags = @(& git tag --list 'PENTIMENTO_V[0-9]*.[0-9]*.[0-9]*[-+][0-9]*' --sort=v:refname)
...
$version = $latestTag.TrimStart('PENTIMENTO_V')
...
Write-Host ("+" * 30)
Write-Host "Building Pentimento plugin..."
Write-Host ("+" * 30)
...
& .\build_pentimento_installer.ps1 $version
Pop-Location

if ($doDeploy) {
  & .\deploy_pentimento.ps1 -Version $version
}
