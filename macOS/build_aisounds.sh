...
VERSION_HEADER_FILE=$NATIVE_ROOT/plugins/aisounds/Source/version.h
...
tags=("${(@f)$(git tag --list 'AISOUNDS_V[0-9]*.[0-9]*.[0-9]*[-+][0-9]*' --sort=v:refname)}")
...
sudo rm -rf /Library/Application\ Support/Avid/Audio/Plug-Ins/aisounds.aaxplugin
...
$PROJUCER --resave $NATIVE_ROOT/plugins/aisounds/aisounds.jucer 

./build_aisounds_plugin_vst.sh
./build_aisounds_plugin_au.sh
./build_aisounds_plugin_aax.sh

IRS_ROOT_AISOUNDS=./installer/irs_root_aisounds
IRS_DEST_AISOUNDS="$IRS_ROOT_AISOUNDS/Library/Application Support/com.dbdone.aisounds/"

rm -rf $IRS_ROOT_AISOUNDS/*
mkdir -p $IRS_DEST_AISOUNDS

echo copying IRs...
cp -R ../common/aisounds/IRs $IRS_DEST_AISOUNDS


WAVETABLES_ROOT_AISOUNDS=./installer/wavetable_root_aisounds
WAVETABLES_DEST_AISOUNDS="$WAVETABLES_ROOT_AISOUNDS/Library/Application Support/com.dbdone.aisounds/"

rm -rf $WAVETABLES_ROOT_AISOUNDS/*
mkdir -p $WAVETABLES_DEST_AISOUNDS

echo copying wavetables...
cp -R ../common/aisounds/wavetables $WAVETABLES_DEST_AISOUNDS


BASIC_ROOT_AISOUNDS=./installer/basic_root_aisounds 
BASIC_DEST_AISOUNDS="$BASIC_ROOT_AISOUNDS/Library/Application Support/com.dbdone.aisounds/"

rm -rf $BASIC_ROOT_AISOUNDS/*
mkdir -p $BASIC_DEST_AISOUNDS

echo copying basic...
cp ../../native/components/dbDoneBackend/Lib/dbdone_backend.dylib $BASIC_DEST_AISOUNDS



pushd .
cd $INSTALLER_DIR 
./build_aisounds_installer.sh $version
popd

if $doDeploy; then
  echo Deploying installer...
  ./deploy_aisounds.sh $version
fi
